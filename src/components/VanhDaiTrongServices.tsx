...
import { VANH_DAI_TRONG_VO_PLANS } from "@/lib/virtualOfficePlans";
...
/**
 * Bảng giá 3 gói BASIC/STANDARD/PRIME — hệ giá riêng, hiện chỉ dùng cho chi
 * nhánh "345 Vành Đai Trong" (xem VANH_DAI_TRONG_VO_PLANS ở
 * virtualOfficePlans.ts). Cùng khuôn mẫu TruongChinhServices.tsx (nhận props
 * branchName/slug/promotions, không hardcode tên chi nhánh) và cùng cách
 * liệt kê ĐẦY ĐỦ từng mục ở cả 3 gói (không có dòng tóm tắt "Toàn bộ tính
 * năng gói thấp hơn").
 */
export default function VanhDaiTrongServices({
  branchName,
  slug,
  promotions,
}: {
    ...
    <section className="py-9">
      <div className="mx-auto max-w-[1240px] px-5 sm:px-8">
        <SectionHead
          ...
          description={`Chi nhánh ${branchName} áp dụng 3 gói văn phòng ảo RIÊNG BIỆT (BASIC, STANDARD, PRIME), khác với hệ thống LITE–RISE chung của MAX OFFICE. Các dịch vụ khác vẫn theo bảng giá chung.`}
        />
        ...
        <Reveal className="mb-6 rounded-2xl border border-line bg-white p-6 sm:p-7">
          <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex shrink-0 items-center gap-3">
              <QuotePlanMenu slug={slug} plans={VANH_DAI_TRONG_VO_PLANS} />
              <Link
          ...
          <RevealGroup className="grid grid-cols-1 gap-5 sm:grid-cols-3">
            {VANH_DAI_TRONG_VO_PLANS.map((plan) => (
