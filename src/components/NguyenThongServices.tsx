...
import { NGUYEN_THONG_VO_PLANS, NGUYEN_THONG_VAT_NOTE, NGUYEN_THONG_ADDONS } from "@/lib/virtualOfficePlans";
...
function BranchPriceBadge() {
  return (
    <span className="inline-flex items-center rounded-full bg-accent/10 px-3 py-1 text-[11.5px] font-bold text-accent">
      Giá áp dụng riêng tại chi nhánh 60 Nguyễn Thông
    </span>
  );
}

export default function NguyenThongServices() {
  return (
    <section className="py-9">
      <div className="mx-auto max-w-[1240px] px-5 sm:px-8">
        <SectionHead
          eyebrow="Dịch vụ tại chi nhánh"
          title="Bảng giá riêng tại chi nhánh 60 Nguyễn Thông"
          description="Chi nhánh 60 Nguyễn Thông áp dụng 4 gói văn phòng ảo RIÊNG BIỆT (SAVE, SILVER, GOLD, PREMIUM), khác với hệ thống LITE–RISE chung của MAX OFFICE. Các dịch vụ khác vẫn theo bảng giá chung."
        />

        {/* Văn phòng ảo — SAVE / SILVER / GOLD / PREMIUM */}
        <Reveal className="mb-6 rounded-2xl border border-line bg-white p-6 sm:p-7">
          ...
          <RevealGroup className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {NGUYEN_THONG_VO_PLANS.map((plan) => (
          ...
          <p className="mt-4 text-[12px] text-body-text italic">{NGUYEN_THONG_VAT_NOTE}</p>
