...
export function getAllOfferedPlans(): OfferedPlan[] {
  ...
  for (const loc of LOCATIONS_LIST) {
    ...
    if (slug === "pham-van-dong") {
      ...
      for (const p of SILVER_GOLD_PREMIUM_Q1Q3_PLANS as SilverGoldPremiumQ1Q3Plan[]) {
        result.push({
          locationSlug: slug,
          locationName: name,
          area,
...
export type PlanGroupLocation = {
  ...
  shortAddress: string;
  area: { slug: string; name: string };
  /** Khuyến mãi riêng của CHI NHÁNH này (từ `LOCATIONS_DATA[slug].promotions`) — không phải của cả nhóm gói, vì trong 1 nhóm mỗi chi nhánh có thể có ưu đãi khác nhau. Undefined/rỗng nếu chi nhánh không có khuyến mãi nào được ghi nhận. */
  promotions?: string[];
};
